...
[CmdletBinding()]
Param (
    [string]$SearchBase = "DC=garbutt-elliott,DC=lan"
)
...
$Computers = Get-ADComputer -Filter {Operatingsystem -like '*Windows 10*' } -SearchBase $SearchBase -Properties LastLogonDate
$Count = 1
...
$ReportPath = '\\GE-FILESVR\share\IT\BitLockerStatus.csv'
Write-Verbose "Building the report..." -Verbose
$Results | Select-Object ComputerName,UserName,LastLogonDate,BitLockerPasswordSet | Sort-Object ComputerName | Export-Csv $ReportPath -NoTypeInformation
